...
cp_r = 1000                 # Rock heat capacity [J/kg/K]
kappa = 3.5                 # Thermal conductivity [J/s/m/K]
...
tmax = 5e6 * solver.t_yr

solver.construct_kernels()
kernels = solver.kernels["psi"]
print(kernels)

solver.run_simulation(T0, tmax)
data = solver.read()
# solver.animate(data, T_range=(400, 600), v_range=(-10, -9))

# t = data["t"].unique()
# inds = data["t"] == t[len(t)//6]
# solver.plot_frame(inds, T_range=(400, 600), v_range=(-10, -9))


u_r = data["u_r"]
u_theta = data["u_theta"]

dr = params["R"] / N_r
r = i*dr

div_u = (1.0/r) * kernels["psi"]["grad_r"].dot(r*u_r) + kernels["psi"]["grad_theta"].dot(u_theta)
div_u = div_u.reshape((-1, N_r*N_theta))
